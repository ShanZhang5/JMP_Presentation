...
- Zhang (2020) uses spatial analysis and finds that pm2.5 levels only increased 0.1 on average after the GS policy around
...
- What are the <span style="color:darkgreen">**Distributional Effects**</span> of the GS policy on pollution relocation for local communities (at census block levels) in the state of California? 
...
- Why Methane Emission?
 ...
 - It can serve as a proxy for other types pollution.
...
$$log(Y_{ijt}) = \alpha + \beta_{1} log(Dist_{ij}) + \beta_{2} log(R_{i}) + \beta_{3} log(R_{j}) + \beta_{4} log(X_{it}) +$$
$$\beta_{5} log(X_{jt}) + \epsilon_{o
} + \theta_{d} + \mu_{od} + \eta_{t} + \lambda_{odt}$$
